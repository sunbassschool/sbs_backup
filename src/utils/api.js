...
export async function hasUserEverLoggedIn() {
...
export async function getPrenomFromIndexedDB() {
    return new Promise((resolve, reject) => {
        const request = indexedDB.open("authDB", 1);
        request.onerror = () => {
        ...
        request.onsuccess = () => {
            ...
            getRequest.onerror = () => {
                console.error("❌ Erreur lecture prénom");
                reject(null);
            };
        };
    });
...
return result?.jwt ?? null; // ✅ juste le jwt
    }
    try {
        sessionStorage.setItem("refreshInProgress", "true");
isRefreshing = refreshToken().then(r => r?.jwt ?? null); // ✅ extrait juste le jwt
...
async function waitForRefresh(timeout = 5000) {
...
export async function storeUserInfo(userData) {
...
async function saveTokensToIndexedDB(jwt, refreshToken) {
    try {
        const db = await openDB("AuthDB", 1);
        const tx = db.transaction("authStore", "readwrite");
        const store = tx.objectStore("authStore");
...
export function shouldRefreshJwt(jwt) {
    if (!jwt || !jwt.includes(".")) {
...
export async function restoreTokensToIndexedDBIfMissing() {
    console.log("🔄 Vérification et restauration des tokens dans IndexedDB...");
    try {
        const db = await openDB("AuthDB", 1);
...
export function setTokenCookies(jwt, refreshToken) {
    if (refreshToken) {
...
export async function restoreAllTokens() {
    ...
    if (!jwt)
        jwt = document.cookie.split("; ").find(row => row.startsWith("jwt="))?.split("=")[1] || null;
    if (!refreshToken)
...
export function isJwtExpired(token) {
    if (!token)
        return true;
    // Récupérer la valeur du token sans la modifier directement
    const jwtString = typeof token === "object" && token.value ? token.value : token;
    // Vérification si le token est bien une chaîne de caractères et a le bon format
    ...
    try {
        ...
        // Fonction robuste pour décoder Base64URL
        ...
        // Vérification de la présence du champ exp et de son format
        if (typeof decodedPayload.exp !== "number") {
            console.warn("⚠️ [isJwtExpired] Champ 'exp' absent ou malformé.");
            return true;
        }
        // Vérification de l'expiration avec un leeway (tolérance)
        const leeway = 10; // 10 secondes de tolérance
        if (decodedPayload.exp * 1000 < Date.now() - leeway * 1000) {
            console.log("⚠️ [isJwtExpired] Token expiré.");
            return true;
        }
        console.log("✅ [isJwtExpired] Token valide.");
        return false;
    }
    catch (error) {
...
export async function checkAndRestoreTokens() {
    const store = useAuthStore(); // si ce n'est pas encore fait
    store.isRefreshingToken = true;
    ...
    try {
        ...
        console.error("💥 Erreur dans checkAndRestoreTokens", err);
        return "error";
    } finally {
...
const storageManager = {
    async getTokenFromAllStorages(key) {
    ...
    setTokenInAllStorages(key, token) {
    ...
    removeTokenFromAllStorages(key) {
...
export async function getItemFromStore(storeName, key) {
    try {
...
export async function saveItemToStore(storeName, key, value) {
    try {
...
export function putItemInStore(store, key, value) {
    return new Promise((resolve, reject) => {
...
export async function restoreTokensIfNeeded() {
...
export async function preventIndexedDBCleanup() {
    if (indexedDBCleanupRunning)
        return; // 🚀 Évite plusieurs instances simultanées
...
export async function checkIndexedDBStatus() {
    try {
        ...
        const err = error; // ✅ Correction du typage
...
return result?.jwt ?? null; // ✅ juste le jwt
    ...
    isRefreshing = new Promise((resolve) => {
    ...
    try {
        ...
        if (token) {
            if (!isJwtExpired(token)) {
                ...
                console.warn("⚠️ Token trouvé mais expiré, on continue le refresh...");
            }
        }
        console.log("⚠️ Aucun JWT valide trouvé. Tentative de rafraîchissement...");
        ...
        if (data.status === "success" && data.data?.jwt && data.data?.refreshToken) {
            ...
            try {
                const decoded = JSON.parse(atob(jwt.split('.')[1]));
                console.log("📌 [DEBUG] Nouveau token exp :", decoded.exp, new Date(decoded.exp * 1000).toISOString());
                if (isJwtExpired(jwt)) {
                ...
                console.error("❌ Impossible de décoder le JWT reçu :", e);
            }
            // ✅ Mise à jour des stockages
            window.dispatchEvent(new Event("jwt-refreshed"));
            await removeItemFromStore("authStore", "jwt");
            await syncAllStorages(jwt, data.data.refreshToken);
...
function removeItemFromStore(key, subKey) {
    if (subKey) {
    ...
    if (storedRefreshToken) {
        try {
            ...
            if (responseData.status === "success") {
                const newJwt = responseData.data.jwt;
...
async function syncAllStorages(jwt, refreshToken) {
    ...
    // 🔹 Cookies (si supprimés)
    document.cookie = `jwt=${jwt}; Secure; SameSite=None; path=/`;
...
// Fonction pour mettre à jour les tokens dans tous les stockages
export async function updateTokens(newJwt, newRefreshToken) {
...
export async function updateRefreshTokenInDB(newRefreshToken) {
    if (!newRefreshToken)
        return;
    try {
        const db = await openDB("AuthDB", 1);
        const tx = db.transaction("authStore", "readwrite");
        const store = tx.objectStore("authStore");
        // ✅ Toujours insérer avec `id: "refreshToken"`
...
export async function updateJWTInIndexedDB(newJwt) {
    if (!newJwt)
        return;
    try {
        const db = await openDB("AuthDB", 1);
