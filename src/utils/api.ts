...
export async function getToken(): Promise<string | null> {
  ...
  if (cookieToken) {
    const value = cookieToken.split("=")[1];
  ...
  if (typeof fromDb === "string" && fromDb.length > 10) {
    return fromDb;
  }
...
export async function getJWTFromIndexedDB(): Promise<string | null> {
  try {
    ...
    if (!db.objectStoreNames.contains("authStore")) {
      return null; // Pas de spam console
    }

    const tx = db.transaction("authStore", "readonly");
    const store = tx.objectStore("authStore");

    const value = await store.get("jwt");
    ...
    console.error("❌ Erreur lecture JWT dans IndexedDB", e);
    return null;
  }
}


async function getSafeAuthDB() {
  try {
    const db = await getAuthDB();

    // Vérifie si la DB est encore utilisable
    if (db === null || db.version === 0) {
      throw new Error("DB non initialisée");
    }

    // Tentative d'opération pour détecter si la DB est cassée ou déjà fermée
    try {
      const tx = db.transaction("authStore", "readonly"); // Remplace "auth" par ton vrai store
      tx.abort(); // On n'exécute rien, juste un test d'ouverture possible
    } catch {
      throw new Error("DB semble fermée ou corrompue");
    }

    return db;

  } catch (err) {
    console.warn("⚠️ DB invalide, reset nécessaire :", err);
    await resetIndexedDB();
    return await getAuthDB(); // retry après reset
  }
}
...
export async function getRefreshTokenFromDB(): Promise<string | null> {
  ...
  let storedRefreshToken: string | null = localStorage.getItem("refreshToken")?.trim() ?? null;

  if (!storedRefreshToken) {
    console.log("🔍 Aucun refresh token trouvé dans localStorage, vérification dans sessionStorage...");
    storedRefreshToken = sessionStorage.getItem("refreshToken")?.trim() ?? null;
  }

  if (!storedRefreshToken) {
    console.log("🔍 Vérification dans IndexedDB...");
    try {
      const db = await getAuthDB();
      const tx = db.transaction("authStore", "readonly");
      const store = tx.objectStore("authStore");
...
storedRefreshToken =
  typeof result?.value === "string" && result.value.length > 10
    ? result.value
    : null;
...
const jwtString = typeof newJwt === "string" ? newJwt : (newJwt as RefreshResponse)?.jwt ?? "";
...
export async function handleRefreshFailure() {
  console.warn("🚨 Refresh token invalide ou expiré. Déconnexion forcée...");

const authStore = useAuthStore();
authStore.refreshFailed = true;

  const event = new CustomEvent("session-expired", {
    detail: "⚠️ Votre session a expiré. Veuillez vous reconnecter.",
  });
  window.dispatchEvent(event);

  await new Promise(resolve => setTimeout(resolve, 2000));

  await logoutUser(); // 🔥 Déconnexion explicite ici
}



export async function checkForStoredTokens() {
  const jwtFromLocal = localStorage.getItem("jwt");
  const refreshFromLocal = localStorage.getItem("refreshToken");

  return !!(jwtFromLocal || refreshFromLocal);
}
...
import { closeAuthDB } from '@/utils/indexedDbUtils';  // ou accès à ton cache global
...
export async function clearUserData() {
  ...
  // ➤ Suppression propre de IndexedDB
// ➤ Suppression propre de IndexedDB
try {
  // Fermer proprement la DB si accessible
  try {
    const db = await getAuthDB();
    if (db) db.close();
  } catch {}
  ...
  deleteRequest.onerror = (event) => {
    console.error("❌ Échec suppression IndexedDB :", event);
  };
  deleteRequest.onblocked = () => {
    console.warn("⚠️ Suppression bloquée — ferme les autres onglets.");
    alert("❗ Suppression bloquée. Ferme les autres onglets ou fenêtres de l'app.");
  };

} catch (error) {
  console.error("❌ Erreur critique dans clearUserData:", error);
}
...
export function scheduleJwtRefresh() {
  // Ne bloque plus le refresh juste à cause d'un ancien logout ou échec
// if (isLoggingOut || refreshFailed) {
//   console.log("⏹ Pas de token valide : logout en cours");
//   return null;
// }
