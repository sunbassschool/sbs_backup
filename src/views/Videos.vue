...
<div v-else-if="role === 'adherent'" class="alert alert-success text-center">
  ✅ Tu es abonné ! Tu as <strong>accès complet</strong> à toutes les vidéos disponibles. 🎉  
</div>
...
let rawRole = decoded?.role || localStorage.getItem("role") || "guest";
...
console.log("✅ Rôle normalisé :", role.value);
    ...
    console.log("⚠️ Aucun JWT trouvé, rôle par défaut : guest");
    role.value = "guest";
  }

  // 🔥 Appeler fetchVideos après avoir défini le rôle
  await fetchVideos();
});
